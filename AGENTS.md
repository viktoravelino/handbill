...
- Keep it small. **The Worker budget is 1310 lines of source** — every `.ts` file in `apps/worker/src` except `*.test.ts`, counted whole, comments included; tests grow freely. It was 750 through 0.2, 1125 from #88, and was raised a second time ahead of M16 (#85). #88 was right that hosted mode earns a raise and right to itemise the cost, and wrong about the size of it, because it priced services that did not exist yet: it put all of 0.3's Worker work at about 315 lines, and M13 (#111) and M14 (#113) alone spent 390 of them (719 → 1109) — M13 cost 223 where #88 allowed 105 for accounts auth and the key routes — leaving 16 for M16 after M15 (#114) came in at zero. This raise is measured against the hosted services now in the tree rather than against the ones that preceded them: M16 costs about 200 lines — the quota service beside `Index` (~95), the check and counter writes in `publish`/`remove` plus the per-tier plumbing decision 11 asks for (~34), the `o:<owner>:<digest>` key index in `auth.ts` deferred from #111's review (~28), the admin takedown route and its `ADMIN_TOKEN` gate (~28), and the wiring in `config.ts`/`index.ts` (~15) — so the 201 lines this raise opens up are again spent down to one. `QuotaExceeded` and every route definition live in `packages/contract`, which this budget does not count, and the WAF rules are a runbook rather than code. 0.4's billing webhook is deliberately _not_ carried here: pricing it two versions out is what made 1125 look sufficient, so it gets re-measured against 0.3's real code when 0.4 starts. `bun run --cwd apps/worker size` prints the count and fails over the budget, and the CI `size` job runs it. It is a budget, not a target: once it binds, a PR that adds lines takes something out and says what. If a feature needs a framework, the feature is wrong.
...
apps/worker         Effect on Workers; one file per service — storage / auth / aliases (+ index in 0.3)
...
- Swappable layers: `StorageR2` / `StorageMemory` and `AliasesKV` / `AliasesDisabled` today, `AuthSecret` / `AuthAccounts` and `IndexBucket` / `IndexKV` when 0.3 adds accounts. A binding that may be missing picks its layer in `index.ts`; the handlers never ask whether a feature is on. Tests run on the memory layers; no Miniflare, no network.
...
- The board is https://github.com/users/viktoravelino/projects/8. Use the `board` skill (`.claude/skills/board/`, also linked from `.agents/skills/`): `board.sh start <n>` when you begin, `board.sh add <n>` for every issue you create, `board.sh review <n>` right after opening the PR, `board.sh show` to see the state. Columns: Todo → In Progress → In Review → Ready to Merge → Done. Nothing reviews a PR automatically here, so review your own diff before you open one — `/code-review` if your tool has it, otherwise read the diff and say in the PR body what you looked for. The project's own workflows move the card to Ready to Merge when the maintainer approves and to Done when the PR merges.
