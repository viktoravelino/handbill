...
- Keep it small. **The Worker budget is 1460 lines of source** — every `.ts` file in `apps/worker/src` except `*.test.ts`, counted whole, comments included; tests grow freely. `QuotaExceeded` and every route definition live in `packages/contract`, which this budget does not count, and the WAF rules are a runbook rather than code. The 1460 is 0.3's real 1308, minus 18 lines of trims, plus 164 itemised for 0.4 on issue #130 — webhook 76, owner key flip 24, paid row 3, admin tier 15, account route 26, CORS 12, secret plumbing 8 — rounded up from 1454; 0.5 is not pre-funded. `bun run --cwd apps/worker size` prints the count and fails over the budget, and the CI `size` job runs it. It is a budget, not a target: once it binds, a PR that adds lines takes something out and says what. If a feature needs a framework, the feature is wrong.
