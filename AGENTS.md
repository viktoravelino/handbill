...
- Keep it small. **The Worker budget is 1560 lines of source** — every `.ts` file in `apps/worker/src` except `*.test.ts`, counted whole, comments included; tests grow freely. `QuotaExceeded` and every route definition live in `packages/contract`, which this budget does not count, and the WAF rules are a runbook rather than code. The 1460 was 0.3's real 1308, minus 18 lines of trims, plus 164 itemised for 0.4 on issue #130 — webhook 76, owner key flip 24, paid row 3, admin tier 15, account route 26, CORS 12, secret plumbing 8 — rounded up from 1454. M20 (#132) re-argued it to 1560: the account route with the usage counters behind it came to 47 rather than the 26 itemised, and the server-created checkout — a security requirement found after #130 — to 48, measured at 1551 on that branch, plus the 12 M21 still owes for CORS. 0.5 is not pre-funded. `bun run --cwd apps/worker size` prints the count and fails over the budget, and the CI `size` job runs it. It is a budget, not a target: once it binds, a PR that adds lines takes something out and says what. If a feature needs a framework, the feature is wrong.
...
- Deploying production is `bun run --cwd apps/worker deploy:prod`, which reads `wrangler.production.jsonc` — the maintainer's copy of the template with the `ALIASES` and `ACCOUNTS` bindings filled in. A bare `wrangler deploy` reads the template, ships without those bindings, and flips handbill.dev back to secret mode; never run it against production. `deploy:prod` (`scripts/deploy-prod.sh`) refuses an untagged HEAD, so production only ever deploys from a release tag reachable from `origin/main`; `--force` skips that gate and is for a hotfix or the kill-switch drill and nothing else — see `docs/RELEASING.md`. Staging deploys itself: `.github/workflows/deploy-worker-staging.yml` runs `deploy:staging` on every push to `main` that touches the Worker or the contract, reading `wrangler.staging.jsonc` — a second Worker on `handbill-staging.dev` with its own bucket and KV namespaces, and the only place the Polar sandbox values live. `bun run --cwd apps/worker deploy:staging` is there for a manual run.
