...
- Deploying production is `bun run --cwd apps/worker deploy:prod`, which reads `wrangler.production.jsonc` — the maintainer's copy of the template with the `ALIASES` and `ACCOUNTS` bindings filled in. A bare `wrangler deploy` reads the template, ships without those bindings, and flips handbill.dev back to secret mode; never run it against production.
