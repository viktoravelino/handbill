...
- Deploying production is `bun run --cwd apps/worker deploy:prod`, which reads `wrangler.production.jsonc` — the maintainer's copy of the template with the `ALIASES` and `ACCOUNTS` bindings filled in. A bare `wrangler deploy` reads the template, ships without those bindings, and flips handbill.dev back to secret mode; never run it against production. Staging deploys itself: `.github/workflows/deploy-worker-staging.yml` runs `deploy:staging` on every push to `main` that touches the Worker or the contract, reading `wrangler.staging.jsonc` — a second Worker on `handbill-staging.dev` with its own bucket and KV namespaces, and the only place the Polar sandbox values live. `bun run --cwd apps/worker deploy:staging` is there for a manual run.
