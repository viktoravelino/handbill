...
#                                       apps/worker/package.json, bun.lock and the contract's
#                                       OpenAPI `version` (with its snapshot), run the checks,
...
# So does the OpenAPI document, which is generated from the contract: a client
# generated against a deployment should name the release it was generated from.
# The spec snapshot holds that string too, so both move or `bun test` below fails.
CONTRACT="$ROOT/packages/contract/src/api.ts"
CONTRACT_SNAP="$ROOT/packages/contract/src/__snapshots__/api.test.ts.snap"
...
bump() {
  ...
  if [[ $DRY == 1 ]]; then
    ...
    trap 'git -C "$ROOT" checkout -q -- apps/cli/package.json apps/worker/package.json bun.lock packages/contract/src/api.ts packages/contract/src/__snapshots__/api.test.ts.snap; echo "dry-run: restored the two package.json files, bun.lock and the contract version; nothing was committed"' EXIT
  fi
  ...
  # The contract has no package.json version of its own (it is never published),
  # so the OpenAPI `version:` line is edited in place, and the snapshot with it.
  perl -pi -e 's/^(      version: )"[^"]+"/$1"'"$v"'"/' "$CONTRACT"
  perl -pi -e 's/^(    "version": )"[^"]+"/$1"'"$v"'"/' "$CONTRACT_SNAP"
  grep -q "version: \"$v\"" "$CONTRACT" || die "could not set the OpenAPI version in packages/contract/src/api.ts"
  ...
  run git -C "$ROOT" add apps/cli/package.json apps/worker/package.json bun.lock \
    packages/contract/src/api.ts packages/contract/src/__snapshots__/api.test.ts.snap
