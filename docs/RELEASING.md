...
Before any release that announces the paid tier: `POLAR_API` and `POLAR_PRODUCT_ID` in `apps/worker/wrangler.production.jsonc` still point at Polar's sandbox organisation, and have to be flipped to the production one — with a production `POLAR_ACCESS_TOKEN` and `POLAR_WEBHOOK_SECRET` put as secrets — or every checkout the CLI hands out takes no money.
