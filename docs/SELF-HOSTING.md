...
- add `Account · Workers R2 Storage · Edit` and `Zone · DNS · Edit`
...
`bunx wrangler delete` removes the Worker and its routes. Delete the bucket in the dashboard (it must be empty first) and the two DNS records. Every published link stops resolving.
