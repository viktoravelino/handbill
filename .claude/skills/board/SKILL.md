...
5. **Open a PR with `Closes #<n>` in the body, then `board.sh review <n>`.** Copilot reviews every PR automatically and re-reviews each push. The `Board` workflow moves the card from the PR's review state: **Ready to Merge** when the maintainer approves the head commit, back to **In Review** on a push or a changes-requested review. Copilot's review is informational — address its findings, but the maintainer's approval is what moves the card. If Copilot left comments: address or answer each one, push, and wait for the next review. Do **not** run `board.sh ready` or `board.sh done` yourself — the automation and the merge are the source of truth. `main` only accepts squash merges via PR with the `typecheck`, `lint`, `test`, and `size` jobs green. **Never merge the PR yourself and never enable auto-merge** — opening it with green checks is the end of your job; the maintainer merges.
