...
5. **Review your own diff, open a PR with `Closes #<n>` in the body, then `board.sh review <n>`.** No bot reviews a PR here, so the review that catches anything is the one you run before opening it: `/code-review` if your tool has it, otherwise read the diff yourself and put what you checked in the PR body. Do not wait for a review to arrive — none will. The project's built-in workflows do the rest: the maintainer's approval moves the card to **Ready to Merge**, a changes-requested review moves it back to **In Review**, and merging moves it to **Done** — never run `board.sh ready` or `board.sh done` yourself. `main` only accepts squash merges via PR with the `typecheck`, `lint`, `test`, and `size` jobs green. **Never merge the PR yourself and never enable auto-merge** — opening it with green checks is the end of your job; the maintainer merges.
