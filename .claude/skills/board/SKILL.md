...
5. **Open a PR with `Closes #<n>` in the body, then `board.sh review <n>`.** Copilot reviews every PR automatically and re-reviews each push. The `Board` workflow reconciles the card from the PR's review state — **Ready to Merge** when a Copilot review on the head commit is clean or a human approves, **In Review** otherwise — on every push and every 15 minutes (`reconcile.sh`; run it yourself with a PR number if you cannot wait). If Copilot left comments: address or answer each one, push, and wait for the next review. Do **not** run `board.sh ready` or `board.sh done` yourself — the automation and the merge are the source of truth. `main` only accepts squash merges via PR with the `typecheck`, `lint`, `test`, and `size` jobs green. **Never merge the PR yourself and never enable auto-merge** — opening it with green checks is the end of your job; the maintainer merges.
