...
5. **Open a PR with `Closes #<n>` in the body, then `board.sh review <n>`.** Copilot reviews every PR automatically and re-reviews each push; address or answer each finding, push, and wait for the next review. The project's built-in workflows do the rest: the maintainer's approval moves the card to **Ready to Merge**, a changes-requested review moves it back to **In Review**, and merging moves it to **Done** — never run `board.sh ready` or `board.sh done` yourself. `main` only accepts squash merges via PR with the `typecheck`, `lint`, `test`, and `size` jobs green. **Never merge the PR yourself and never enable auto-merge** — opening it with green checks is the end of your job; the maintainer merges.
