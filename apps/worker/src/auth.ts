 ...
 * page owned by `"self"`. There are no accounts here, so the two key routes fail
 * with `NotFound` exactly as `AliasesDisabled` fails the alias routes — the
 ...
 * renaming themselves. GitHub refusing the token is a plain 401; GitHub being
 * unreachable rejects and surfaces as a 500, so login fails loudly while
 * publishing — which never calls this — carries on.
 */
...
export const AuthAccounts = (
  ...
  Layer.succeed(Auth, {
    ...
    // Idempotent: a key already revoked, or never minted, is a 204 as well. The
    // record stays, so the revocation is on the books and pages keep serving.
    revoke: (candidate) =>
      Effect.gen(function* () {
