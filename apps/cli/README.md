...
| `handbill account`                        | Owner, tier and the quotas spent today; `--json` for the object. `--upgrade` prints a checkout URL for the paid tier, `--open` opens it. `--web` opens the same account in the browser, on the site of an `api.<zone>` deployment. |
