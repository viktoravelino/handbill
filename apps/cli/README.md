...
One command for the whole rotation: publish the new file, re-point every name that pointed at the old page, then unpublish the old hash — in that order, so a reader following a name never lands on a 404. It prints the new URL and nothing else; `--json` adds what it did (`{ "hash", "url", "created", "removed", "aliases" }`), and the names it moved are reported on stderr. Update a page to bytes it already has and nothing happens.
