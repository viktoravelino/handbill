 ...
 * after what it acts on — `pages.ts`, `aliases.ts`, `update.ts`, `doctor.ts`,
 * `completions.ts` — and built from the helpers in `command-kit.ts`. The order
 * below is the order `--help` lists them in.
 */
