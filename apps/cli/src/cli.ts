...
import { GitHubDeviceLive } from "./github"
 ...
 * whatever the platform opens URLs with, and `login` talks to GitHub over the
 * same transport as the API.
 */
const program = Command.run(handbill, { version }).pipe(
  Effect.provide(
    Layer.mergeAll(
      ...
      BrowserLive,
      QrLive,
      GitHubDeviceLive.pipe(Layer.provide(FetchHttpClient.layer))
    )
  )
)
